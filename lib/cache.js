...
module.exports = function(options) {
    ...
    if (!cache.get('blog')) {
      request.get(properties.apiKeys.blog.url + '/ghost/api/v0.1/public/posts/', function (error, response, data) {
        ...
        if (error) {
          ...
          cache.put('blog', data, CACHE_TIMEOUT)
          callback(null, data)
        }
      })
    } else {
  ...
  function dribble(callback) {
    var baseUrl = 'https://api.dribbble.com/v1/users/'
      , user = properties.apiKeys.dribbble.user
      , accessToken = '?access_token=' + properties.apiKeys.dribbble.token

    if (!cache.get('dribble')) {
      request.get( baseUrl + user + '/shots' + accessToken, function (error, response, data) {
        logger.info('Getting Dribble shots')
        if (error) {
          logger.error(error)
        } else {
          data = trimDribble(JSON.parse(data))
          cache.put('dribble', data, CACHE_TIMEOUT)
          callback(null, data)
        }
      })
    } else {
    ...
    if (!cache.get('instagram')) {
      request.get(baseUrl + token, function (error, response, data) {
        ...
        if (error) {
          ...
          cache.put('instagram', data, CACHE_TIMEOUT)
          callback(null, data)
        }
      })
    } else {
    ...
    if (!cache.get('twitter')) {
      twitterClient.get('statuses/user_timeline', function(error, params){
        ...
        if (error) {
          ...
          cache.put('twitter', params, CACHE_TIMEOUT)
    ...
    if (!cache.get('github')) {
      request.get(options, function (error, response, data) {
        ...
        if (error) {
          ...
          cache.put('github', data, CACHE_TIMEOUT)
          callback(null, data)
        }
      })
    } else {
