...
module.exports = function(options) {
  ...
  function tumblr (callback) {
    var apiKey = properties.apiKeys.tumblr.key
      , baseUrl = 'http://api.tumblr.com/v2/blog/'
      , blog = properties.apiKeys.tumblr.blog

    if (!cache.get('tumblr')) {
      request.get( baseUrl + blog + '/posts/text?api_key=' + apiKey + '&reblog_info=true', function (error, response, data) {
        logger.info('Getting Tumblr posts')
        if (error) {
          logger.error(error)
        } else {
          data = trimTumblr(JSON.parse(data))
          cache.put('tumblr', data, (1000 * 60))
          callback(null, data)
        }
      })
    } else {
      callback(null, cache.get('tumblr'))
    }
  }
  ...
  function trimTumblr (data) {
    var posts = []
    for (var i = 0; i < 5; i++) {
      data.response.posts[i].date = formatDateTime(data.response.posts[i].date)
